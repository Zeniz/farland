...
void image::renderABS(float alpha)
{
	render(CAMERA2D->getCamPosX(), CAMERA2D->getCamPosY(), _imageInfo->width, _imageInfo->height, 0.f, 0.f, _imageInfo->width, _imageInfo->height, alpha);
}
...
void image::renderABS(float destX, float destY, float alpha)
{
	render(destX + CAMERA2D->getCamPosX(), destY + CAMERA2D->getCamPosY(), _imageInfo->width, _imageInfo->height, 0.f, 0.f, _imageInfo->width, _imageInfo->height, alpha);
}
...
void image::renderABS(float destX, float destY, int showWidth, int showHeight, float alpha)
{
	render(destX + CAMERA2D->getCamPosX(), destY + CAMERA2D->getCamPosY(), showWidth, showHeight, 0.f, 0.f, _imageInfo->width, _imageInfo->height, alpha);
}
...
void image::renderABS(float destX, float destY, float sourX, float sourY, int sourWidth, int sourHeight, float alpha)
{
	render(destX + CAMERA2D->getCamPosX(), destY + CAMERA2D->getCamPosY(), sourWidth, sourHeight, sourX, sourY, sourWidth, sourHeight, alpha);
}
...
void image::renderABS(float destX, float destY, int showWidth, int showHeight, float sourX, float sourY, int sourWidth, int sourHeight, float alpha)
{
	POINTFLOAT pf = GetRenderPosition(destX + CAMERA2D->getCamPosX(), destY + CAMERA2D->getCamPosY());
...
void image::frameRenderABS(float destX, float destY, int currentFrameX, int currentFrameY, float alpha)
{
	frameRender(destX + CAMERA2D->getCamPosX(), destY + CAMERA2D->getCamPosY(), _imageInfo->frameWidth, _imageInfo->frameHeight, currentFrameX, currentFrameY, alpha);
}
...
void image::frameRenderAngleABS(float destX, float destY, int currentFrameX, int currentFrameY, float angle, float alpha)
{
	D2DMANAGER->_renderTarget->SetTransform(D2D1::Matrix3x2F::Rotation(angle, Point2F(destX + _imageInfo->frameWidth / 2, destY + _imageInfo->frameHeight / 2)));
	frameRender(destX + CAMERA2D->getCamPosX(), destY + CAMERA2D->getCamPosY(), _imageInfo->frameWidth, _imageInfo->frameHeight, currentFrameX, currentFrameY, alpha);
	D2DMANAGER->_renderTarget->SetTransform(D2D1::Matrix3x2F::Identity());
...
void image::frameRenderABS(float destX, float destY, int showWidth, int showHeight, int currentFrameX, int currentFrameY, float alpha)
{
	POINTFLOAT pf = GetRenderPosition(destX + CAMERA2D->getCamPosX(), destY + CAMERA2D->getCamPosY());
...
void image::frameRenderABS(float destX, float destY, int sourX, int sourY, int sourWidth, int sourHeight, int currentFrameX, int currentFrameY, float alpha)
{
	POINTFLOAT pf = GetRenderPosition(destX + CAMERA2D->getCamPosX(), destY + CAMERA2D->getCamPosY());
...
void image::frameRenderReverseX(float destX, float destY, int showWidth, int showHeight, int currentFrameX, int currentFrameY, float alpha)
{
	...
	if (_imageInfo->bitmap != NULL)
	{
		...
		size.width = -1;
		size.height = 1;
		D2DMANAGER->_renderTarget->SetTransform(D2D1::Matrix3x2F::Scale(size, Point2F(destX - CAMERA2D->getCamPosX() + _imageInfo->frameWidth / 2, destY - CAMERA2D->getCamPosY() + _imageInfo->frameHeight / 2)));
		D2D1_RECT_F dxArea = RectF(pf.x, pf.y, pf.x + showWidth, pf.y + showHeight);
		D2D1_RECT_F dxArea2 = RectF(currentFrameX * _imageInfo->frameWidth
			, currentFrameY * _imageInfo->frameHeight
			, (currentFrameX + 1) * _imageInfo->frameWidth
...
{
	POINTFLOAT pf = { destX - (int)CAMERA2D->getCamPosX() , destY - (int)CAMERA2D->getCamPosY() };
	return pf;
}
