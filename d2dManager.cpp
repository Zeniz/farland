...
void d2dManager::drawDiamondLine(ID2D1SolidColorBrush * brush, float centerX, float centerY, float wid, float hei, float strokeWid)
{
	D2D1_RECT_F rcf = getDrawRectfArea(centerX - wid / 2, centerY - hei / 2, centerX + wid / 2, centerY + hei / 2);

	if (!isRectFInRangeWindow(rcf))
		return;

	_renderTarget->DrawLine(Point2F(rcf.left + wid / 2, rcf.top), Point2F(rcf.left, rcf.top + hei / 2), brush, strokeWid);			//	상->좌
	_renderTarget->DrawLine(Point2F(rcf.left, rcf.top + hei / 2), Point2F(rcf.left + wid / 2, rcf.bottom), brush, strokeWid);			//	좌->하
	_renderTarget->DrawLine(Point2F(rcf.left + wid / 2, rcf.bottom), Point2F(rcf.right, rcf.top + hei / 2), brush, strokeWid);			//	하->우
	_renderTarget->DrawLine(Point2F(rcf.right, rcf.top + hei / 2), Point2F(rcf.left + wid / 2, rcf.top), brush, strokeWid);			//	우->상
}

void d2dManager::drawDiamondLine(float centerX, float centerY, float wid, float hei)
{
	D2D1_RECT_F rcf = getDrawRectfArea(centerX - wid / 2, centerY - hei / 2, centerX + wid / 2, centerY + hei / 2);
	...
	_renderTarget->DrawLine(Point2F(rcf.left + wid / 2, rcf.top), Point2F(rcf.left, rcf.top + hei / 2), _defaultBrush, 1);			//	상->좌
	_renderTarget->DrawLine(Point2F(rcf.left, rcf.top + hei / 2), Point2F(rcf.left + wid / 2, rcf.bottom), _defaultBrush, 1);			//	좌->하
	_renderTarget->DrawLine(Point2F(rcf.left + wid / 2, rcf.bottom), Point2F(rcf.right, rcf.top + hei / 2), _defaultBrush, 1);			//	하->우
	_renderTarget->DrawLine(Point2F(rcf.right, rcf.top + hei / 2), Point2F(rcf.left + wid / 2, rcf.top), _defaultBrush, 1);			//	우->상
}

void d2dManager::drawDiamondLine(COLORREF rgb, float centerX, float centerY, float wid, float hei)
{
	D2D1_RECT_F rcf = getDrawRectfArea(centerX - wid/2, centerY - hei/2, centerX + wid / 2, centerY + hei / 2);
	...
	_renderTarget->DrawLine(Point2F(rcf.left + wid / 2, rcf.top), Point2F(rcf.left, rcf.top + hei / 2), createBrush(rgb, 1), 1);			//	상->좌
	_renderTarget->DrawLine(Point2F(rcf.left, rcf.top + hei / 2), Point2F(rcf.left + wid / 2, rcf.bottom), createBrush(rgb, 1), 1);			//	좌->하
	_renderTarget->DrawLine(Point2F(rcf.left + wid / 2, rcf.bottom), Point2F(rcf.right, rcf.top + hei / 2), createBrush(rgb, 1), 1);			//	하->우
	_renderTarget->DrawLine(Point2F(rcf.right, rcf.top + hei / 2), Point2F(rcf.left + wid / 2, rcf.top), createBrush(rgb, 1), 1);			//	우->상
}

void d2dManager::drawDiamondLine(COLORREF rgb, float centerX, float centerY, float wid, float hei, float strokeWid)
{
	D2D1_RECT_F rcf = getDrawRectfArea(centerX - wid / 2, centerY - hei / 2, centerX + wid / 2, centerY + hei / 2);
	...
	_renderTarget->DrawLine(Point2F(rcf.left + wid / 2, rcf.top), Point2F(rcf.left, rcf.top + hei / 2), createBrush(rgb, 1), strokeWid);			//	상->좌
	_renderTarget->DrawLine(Point2F(rcf.left, rcf.top + hei / 2), Point2F(rcf.left + wid / 2, rcf.bottom), createBrush(rgb, 1), strokeWid);			//	좌->하
	_renderTarget->DrawLine(Point2F(rcf.left + wid / 2, rcf.bottom), Point2F(rcf.right, rcf.top + hei / 2), createBrush(rgb, 1), strokeWid);			//	하->우
	_renderTarget->DrawLine(Point2F(rcf.right, rcf.top + hei / 2), Point2F(rcf.left + wid / 2, rcf.top), createBrush(rgb, 1), strokeWid);			//	우->상
}
...
{
	D2D1_RECT_F rc;
	rc = {	  startX - CAMERA2D->getCamPosX()
			, startY - CAMERA2D->getCamPosY()
			, endX	 - CAMERA2D->getCamPosX()
			, endY	 - CAMERA2D->getCamPosY()
		};
